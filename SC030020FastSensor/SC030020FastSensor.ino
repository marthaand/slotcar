...
// SC030020FastSensor - Faster slave read
//--------------------------------------------------------------------------------
...
void handleSensorChange(int sensor, int value){     //Dette kan være hvor feilen ligger, registrerer bare start av race hvis value==1...
  if (SensorStatus[sensor]=="NO"){              //THIS MEANS START OF RACE? OR NO CURRENT RACE
    if (value==1){
  ...
  if (SensorStatus[sensor]=="YES"){           //Commencing race?
    if (value==0){                            //What about value 1/0?
      Serial.print ("TRACK:");
      Serial.print (sensor);
      Serial.print (",State:");
...
void handleSerial(){
  ...
  if (avail){
    while (CharAvailable(1)){  // Fast Slave sensors 20190708
      x=getChar(1);
      handleChar(1,x);        
    }                         // Fast Slave sensors 20190708
  }
